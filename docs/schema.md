...
成交紀錄是分批止盈與實際減倉的主要追溯來源。若能提供 `order_time`，系統會用它判斷是否為 2026-05-07 18:30 +08:00 後下的單；沒有 `order_time` 時才使用 `fill_time` 作為 fallback。

Bitget 唯讀 API 匯入時，`fills` 必須能用 `orderId` 對回 `orders-history` 內 18:30 後建立的委託，才會寫入核心 `fills` 表。若委託時間早於 18:30 或無法確認，該成交列會保留在原始 API JSON 與 `import_row_issues`，不會混入核心成交表。
