...
`POST /api/bitget/sync-readonly` 是前端 `/recorded` 使用的自動偵測端點；它會用固定的 `bitget_live_readonly` run 同步台灣時間 2026/05/08 00:00 後的新資料，並用 Bitget `orderId` / `tradeId` 去重，避免刷新時重複計入。
...
`fills` 會用 `orderId` 對回 2026/05/08 00:00 後建立的 Bitget order；若成交發生在 5/8 後、但委託是在 5/8 前建立，該列只保留在 raw files 與 `import_row_issues`，不寫入核心 `fills` 表。
...
- [Get Order Fill Details](https://www.bitget.com/api-doc/classic/contract/trade/Get-Order-Fills)

Request:

```json
{
  "run_id": "bitget_20260508_readonly_001",
