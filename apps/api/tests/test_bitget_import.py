...
from app.services.bitget import BitgetReadOnlyClient, bitget_position_effect
...
def test_settings_enforces_may_8_taipei_record_start():
    settings = Settings(
        DATABASE_URL="sqlite://",
    ...
    assert settings.record_start_at == datetime.fromisoformat("2026-05-08T00:00:00+08:00")


@pytest.mark.parametrize(
    ("row", "expected"),
    [
        ({"tradeSide": "buy_single", "reduceOnly": "NO", "orderSource": "market"}, "open"),
        ({"tradeSide": "sell_single", "reduceOnly": "YES", "orderSource": "market"}, "reduce"),
        ({"tradeSide": "buy_single", "reduceOnly": "YES", "orderSource": "pos_profit_market"}, "partial_take_profit"),
        ({"tradeSide": "buy_single", "reduceOnly": "YES", "orderSource": "pos_loss_market"}, "stop_loss"),
        ({"tradeSide": "close", "reduceOnly": "NO", "orderSource": "market"}, "close"),
    ],
)
def test_bitget_position_effect_avoids_open_close_misclassification(row: dict, expected: str):
    assert bitget_position_effect(row) == expected
