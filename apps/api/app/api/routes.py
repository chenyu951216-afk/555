...
from app.schemas.imports import BitgetImportRequest, BitgetSyncRequest, NoteCreate, NoteUpdate, RunPatch, SafeQueryFilter
...
def bitget_recorded_data(
    ...
    fill_stat_rows = db.execute(
        select(
            FillRecord.symbol,
            FillRecord.position_effect,
            func.count().label("fill_count"),
            func.sum(FillRecord.notional).label("notional"),
            func.sum(FillRecord.fee).label("fee"),
            func.sum(FillRecord.realized_pnl).label("realized_pnl"),
        )
        .join(BacktestRun, BacktestRun.run_id == FillRecord.run_id)
        .where(real_run_filter, FillRecord.fill_time.is_not(None), FillRecord.fill_time >= record_start)
        .group_by(FillRecord.symbol, FillRecord.position_effect)
        .order_by(func.count().desc())
    ).all()
    order_effect_rows = db.execute(
        select(OrderRecord.position_effect, func.count())
        .join(BacktestRun, BacktestRun.run_id == OrderRecord.run_id)
        .where(real_run_filter, OrderRecord.order_time.is_not(None), OrderRecord.order_time >= record_start)
        .group_by(OrderRecord.position_effect)
        .order_by(func.count().desc())
    ).all()
    ...
    return {
        ...
        "stats": {
            "orders_by_effect": [
                {"position_effect": effect or "unknown", "count": count}
                for effect, count in order_effect_rows
            ],
            "fills_by_symbol_effect": [
                {
                    "symbol": symbol or "unknown",
                    "position_effect": effect or "unknown",
                    "fill_count": fill_count,
                    "notional": notional,
                    "fee": fee,
                    "realized_pnl": realized_pnl,
                }
                for symbol, effect, fill_count, notional, fee, realized_pnl in fill_stat_rows
            ],
        },
...
@router.post("/bitget/sync-readonly", response_model=ImportResult)
def sync_bitget_readonly(
    payload: BitgetSyncRequest | None = None,
    db: Session = Depends(get_db),
    settings: Settings = Depends(get_settings),
) -> ImportResult:
    try:
        return BitgetImportService(db, settings).sync_read_only_history(payload or BitgetSyncRequest())
    except ValueError as exc:
        raise HTTPException(status_code=422, detail=str(exc)) from exc
    except httpx.HTTPStatusError as exc:
        raise HTTPException(status_code=502, detail="Bitget API 回傳 HTTP 錯誤，請確認金鑰權限、交易對與時間範圍") from exc
