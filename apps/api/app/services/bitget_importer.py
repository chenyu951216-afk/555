...
from app.schemas.common import ImportResult, ValidationReport
...
class BitgetImportService:
    ...
    def import_read_only_history(self, payload: BitgetImportRequest) -> ImportResult:
        ...
        report = ValidationReport(ok=True, run_id=payload.run_id, record_start_at=record_start, result_hash=result_hash)
    ...
    def _insert_fills(self, run_id: str, rows: list[dict[str, Any]], start_time, order_created_at: dict[str, Any]) -> int:
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if not is_after_record_start(fill_time, start_time):
                self._row_issue(run_id, "bitget.fill-history", index, "outside_record_window", "cTime 早於匯入起點，或時間無法解析", row)
                continue
