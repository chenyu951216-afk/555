...
from app.schemas.imports import BitgetImportRequest, BitgetSyncRequest
...
LIVE_RUN_ID = "bitget_live_readonly"
...
class BitgetImportService:
    ...
    def import_read_only_history(self, payload: BitgetImportRequest) -> ImportResult:
        ...
        try:
            ...
            run = BacktestRun(
                ...
                schema_version="1.0",
                status="active",
                tags=["bitget", "readonly_api", "orders", "fills", "record_start_20260508"],
            ...
            order_effects = bitget_order_effects(all_orders)
            counts["fills"] = self._insert_fills(payload.run_id, all_fills, start_time, order_created_at, order_effects)
            counts["row_issues"] = (
    ...
    def sync_read_only_history(self, payload: BitgetSyncRequest) -> ImportResult:
        record_start = self.settings.record_start_at.astimezone(timezone.utc)
        end_time = utcnow().astimezone(timezone.utc)
        if end_time <= record_start:
            raise ValueError("目前時間尚未晚於最早計入時間")

        start_time = self._sync_start_time(record_start, end_time)
        symbols = payload.symbols or [None]
        all_orders: list[dict[str, Any]] = []
        all_fills: list[dict[str, Any]] = []
        for symbol in symbols:
        ...
        counts = {"orders": 0, "fills": 0, "row_issues": 0, "metrics": 0, "raw_orders": len(all_orders), "raw_fills": len(all_fills)}
        try:
            run = self._ensure_live_run(payload.product_type, record_start, end_time)
            order_created_at = {
                str(row.get("orderId")): ms_to_datetime(row.get("cTime"))
                for row in all_orders
                if row.get("orderId")
            }
            existing_order_times = db_order_times(self.db)
            order_created_at.update({order_id: order_time for order_id, order_time in existing_order_times.items() if order_id not in order_created_at})
            order_effects = db_order_effects(self.db)
            order_effects.update(bitget_order_effects(all_orders))

            counts["orders"] = self._insert_new_orders(LIVE_RUN_ID, all_orders, record_start)
            counts["fills"] = self._insert_new_fills(LIVE_RUN_ID, all_fills, record_start, order_created_at, order_effects)
            counts["row_issues"] = (
                self.db.scalar(select(func.count()).select_from(ImportRowIssue).where(ImportRowIssue.run_id == LIVE_RUN_ID))
                or 0
            )
            self._upsert_metric(LIVE_RUN_ID, "order_count", self.db.scalar(select(func.count()).select_from(OrderRecord).where(OrderRecord.run_id == LIVE_RUN_ID)) or 0)
            self._upsert_metric(LIVE_RUN_ID, "fill_count", self.db.scalar(select(func.count()).select_from(FillRecord).where(FillRecord.run_id == LIVE_RUN_ID)) or 0)
            self._upsert_metric(LIVE_RUN_ID, "bitget_last_raw_order_count", len(all_orders))
            self._upsert_metric(LIVE_RUN_ID, "bitget_last_raw_fill_count", len(all_fills))
            self._upsert_metric(LIVE_RUN_ID, "row_issue_count", counts["row_issues"])
            counts["metrics"] = 5
            run.end_time = end_time
            run.imported_at = utcnow()
            run.result_hash = sha256_json({"orders": all_orders, "fills": all_fills, "start_time": start_time.isoformat(), "end_time": end_time.isoformat()})
            self.db.add(
                AuditLog(
                    actor=self.settings.default_user_email,
                    action="bitget_readonly_sync",
                    target_type="backtest_run",
        ...
        report = ValidationReport(
            ...
            files=[
                FileValidationSummary(file_name="bitget_orders_history.json", file_type="bitget_api_json", row_count=len(all_orders), validation_status="ok"),
                FileValidationSummary(file_name="bitget_fill_history.json", file_type="bitget_api_json", row_count=len(all_fills), validation_status="ok"),
            ],
        )
        return ImportResult(ok=True, run_id=LIVE_RUN_ID, validation=report, imported_counts=counts)
    ...
    def _insert_new_orders(self, run_id: str, rows: list[dict[str, Any]], start_time) -> int:
        existing = set(
            self.db.scalars(
                select(OrderRecord.order_id)
                .join(BacktestRun, BacktestRun.run_id == OrderRecord.run_id)
                .where(BacktestRun.data_source == "bitget_api_v2")
            ).all()
        )
        ...
        for index, row in enumerate(rows, start=1):
            ...
            filtered.append(row)
        return self._insert_orders(run_id, filtered, start_time)
    ...
    def _insert_fills(
        ...
        start_time,
        order_created_at: dict[str, Any],
        order_effects: dict[str, str | None] | None = None,
    ) -> int:
        count = 0
        for index, row in enumerate(rows, start=1):
            ...
            self.db.add(
                FillRecord(
                    ...
                    reduce_only=None,
                    position_effect=(order_effects or {}).get(order_id or "") or bitget_position_effect(row),
    ...
    def _insert_new_fills(
        self,
        run_id: str,
        rows: list[dict[str, Any]],
        start_time,
        order_created_at: dict[str, Any],
        order_effects: dict[str, str | None],
    ) -> int:
        existing = set(
            self.db.scalars(
                select(FillRecord.fill_id)
        ...
        for index, row in enumerate(rows, start=1):
            ...
            filtered.append(row)
        return self._insert_fills(run_id, filtered, start_time, order_created_at, order_effects)

    def _sync_start_time(self, record_start, end_time) -> Any:
        last_order_time = self.db.scalar(select(func.max(OrderRecord.order_time)).where(OrderRecord.run_id == LIVE_RUN_ID))
        last_fill_time = self.db.scalar(select(func.max(FillRecord.fill_time)).where(FillRecord.run_id == LIVE_RUN_ID))
        latest = max([value for value in (last_order_time, last_fill_time) if value is not None], default=None)
        if latest is None:
            lookback_days = max(1, self.settings.bitget_sync_lookback_days)
            return max(record_start, end_time - timedelta(days=lookback_days))
        return max(record_start, latest.astimezone(timezone.utc) - timedelta(minutes=10))

    def _ensure_live_run(self, product_type: str, record_start, end_time) -> BacktestRun:
        run = self.db.scalar(select(BacktestRun).where(BacktestRun.run_id == LIVE_RUN_ID))
        if run is not None:
            return run
        workspace, user = _local_user(self.db, self.settings)
        run = BacktestRun(
            ...
            created_by="bitget_readonly_api",
            data_source="bitget_api_v2",
            data_version=product_type,
            code_version="readonly_sync_v1",
            schema_version="1.0",
            status="active",
            tags=["bitget", "readonly_api", "orders", "fills", "auto_sync", "record_start_20260508"],
            notes="Bitget API 自動唯讀同步；系統沒有實作任何下單端點。",
        )
        self.db.add(run)
        self.db.flush()
        for tag in run.tags:
            self.db.add(RunTag(run_id=LIVE_RUN_ID, tag=tag))
        return run

    def _upsert_metric(self, run_id: str, key: str, value: int) -> None:
        metric = self.db.scalar(select(BacktestMetric).where(BacktestMetric.run_id == run_id, BacktestMetric.metric_key == key))
        if metric is None:
            metric = BacktestMetric(run_id=run_id, metric_key=key, metric_category="bitget_sync")
        metric.metric_value_numeric = value
        self.db.add(metric)
...
def db_order_times(db: Session) -> dict[str, Any]:
    rows = db.execute(
        select(OrderRecord.order_id, OrderRecord.order_time)
        .join(BacktestRun, BacktestRun.run_id == OrderRecord.run_id)
        .where(BacktestRun.data_source == "bitget_api_v2", OrderRecord.order_time.is_not(None))
    ).all()
    return {order_id: order_time for order_id, order_time in rows}


def db_order_effects(db: Session) -> dict[str, str | None]:
    rows = db.execute(
        select(OrderRecord.order_id, OrderRecord.position_effect)
        .join(BacktestRun, BacktestRun.run_id == OrderRecord.run_id)
        .where(BacktestRun.data_source == "bitget_api_v2")
    ).all()
    return {order_id: effect for order_id, effect in rows}


def bitget_order_effects(rows: list[dict[str, Any]]) -> dict[str, str | None]:
    return {
        str(row.get("orderId")): bitget_position_effect(row)
        for row in rows
        if row.get("orderId")
    }
