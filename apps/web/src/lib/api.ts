...
export type BitgetRecordedData = {
  ...
  stats?: {
    orders_by_effect: Array<{ position_effect: string; count: number }>;
    fills_by_symbol_effect: Array<{
      symbol: string;
...
export async function apiFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  const discoveredBase = method === "GET" || method === "HEAD" ? "" : await discoverProxyApiBase();
  const candidates = getApiCandidates(path, method === "GET" || method === "HEAD", discoveredBase);
  const errors: string[] = [];
  ...
  for (const candidate of candidates) {
    try {
      ...
      if (isErrorPayload(payload)) {
        throw new ApiHttpError(payload.detail || payload.message || "API 回傳錯誤");
      }
      return payload as T;
    } catch (error) {
...
function getApiCandidates(path: string, allowDiscovery: boolean, discoveredBase = ""): ApiCandidate[] {
  ...
  addDirect("前台代理提供的後端 API", discoveredBase);
...
async function discoverProxyApiBase() {
  if (typeof window === "undefined") return "";
  const stored = getStoredApiBase();
  if (stored) return stored;
  try {
    const response = await fetchWithTimeout(proxyPath("/__connection"));
    if (!response.ok) return "";
    const payload = await response.json() as ProxyConnectionStatus;
    return normalizeApiBase(payload.base_url || "");
  } catch {
    return "";
  }
}
...
function isErrorPayload(payload: unknown): payload is { detail?: string; message?: string } {
  if (!payload || typeof payload !== "object") return false;
  const value = payload as Record<string, unknown>;
  const hasMessage = typeof value.detail === "string" || typeof value.message === "string";
  const hasSuccessShape = "ok" in value || "configured" in value || "source" in value || "items" in value || "run_id" in value;
  return hasMessage && !hasSuccessShape;
}
