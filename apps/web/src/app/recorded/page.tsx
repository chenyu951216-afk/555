...
import { apiFetch, BitgetRecordedData, BitgetStatus, RecordedAuditLog, RecordedFill, RecordedOrder, RecordedRowIssue } from "@/lib/api";
...
export default function RecordedDataPage() {
  ...
  const dataQuery = useQuery({
    ...
    queryFn: () => apiFetch<BitgetRecordedData>("/bitget/recorded-data?order_limit=1000&fill_limit=1000&issue_limit=200&log_limit=200"),
  });
  ...
  return (
    <div>
      <PageHeader
        title="計入資料"
        description="只顯示 2026-05-07 18:30（台北時間）之後，從真實 Bitget API 匯入並已計入資料庫的訂單與成交。"
        action={
